...
const ProductDetails = ({ data }) => {
  ...
  const averageRating = totalReviewsLength > 0 ? (totalRatings / totalReviewsLength).toFixed(2) : 0;
                  ...
                  {data.images.map((img, index) => (
                    <div key={index} className="cursor-pointer mr-3" onClick={() => setSelect(index)}>
                      <img 
                        src={img.url} 
                        alt="" 
                        className="h-[200px] object-contain rounded-md border-2 border-transparent hover:border-[#cdaa80] transition-colors"
                      />
                    </div>
                  ))}
                </div>
              ...
              <div className="w-full 800px:w-[50%] pt-5">
                ...
                <div className="flex items-center mt-12 justify-between pr-3">
                  <div className="flex items-center">
                    <button
                      className="bg-gradient-to-r from-[#0f1e3f] to-[#213a56] text-white font-bold rounded-l px-4 py-2"
      ...
      <div className="w-full flex justify-between border-b pt-10 pb-2">
        {[
          ...
          <div key={tab.id} className="relative">
            ...
            {active === tab.id && <div className={styles.active_indicator} />}
          </div>
        ))}
      </div>

      {/* Tab Content */}
      {active === 1 && <p className="py-2 text-[18px] leading-8 pb-10 whitespace-pre-line">{data.description}</p>}
        ...
        <div className="w-full block 800px:flex p-5">
          ...
          <div className="w-full 800px:w-[50%] mt-5 800px:mt-0 flex flex-col items-end">
            <div className="text-left">
              ...
              <h5 className="font-[600] pt-3">
                Total Reviews: <span className="font-[500]">{totalReviewsLength || 0}</span>
              </h5>
              <Link to="/">
